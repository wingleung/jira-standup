...
module.exports.fetch = function(config) {
    const { username, project, jiraUrl } = config;
    const jiraUrl = config.jiraUrl && config.jiraUrl.endsWith('/') ? config.jiraUrl.slice(0, -1) : config.jiraUrl;

    if (!username) {
        log.error('No username found, make sure you have .jirarc file with a username property');
        return;
    }

    if (!jiraUrl) {
        log.error('No jiraUrl found, make sure you have .jirarc file with a jiraUrl property');
        return;
    }
    ...
    const jiraRestUrl = `${jiraUrl}/rest/api/2/search?jql=${encodeURIComponent(query)}`;

    const spinner = ora(chalk.blue('Fetching issues...')).start();

    fetch(`${jiraRestUrl}`, {timeout: 5000})
        .then(res => res.json()).then(json => {
            ...
            } else if (json.warningMessages) {
                json.warningMessages.forEach((warning) => log.warning(warning));
            } else if (json.issues && json.issues.length > 0) {
                ...
                assignedIssues.forEach((issue) => {
                    ...
                    if (issue.fields.issuelinks) {
                        ...
                        issueLinks.forEach((issueLink) => {
                            ...
                            const impedementType = (status == 'Closed')

...
module.exports.fetch = function(config) {
  ...
  if (!username) {
    log.error(
      'No username found, make sure you have .jirarc file with a username property'
    );
    return;
  }
  ...
  if (!jiraUrl) {
    log.error(
      'No jiraUrl found, make sure you have .jirarc file with a jiraUrl property'
    );
    return;
  }
  ...
  const jiraRestUrl = `${jiraUrl}/rest/api/2/search?jql=${encodeURIComponent(
    query
  )}`;
    ...
    .then(json => {
      ...
      if (json.errorMessages) {
        ...
        assignedIssues.forEach(issue => {
          ...
          const summary = chalk.red.bold('jira description');

          let status = chalk.yellow(issue.fields.status.name);
          status = impedement
            ? `${status} ${chalk.yellow.dim(impedement)}`
            : status;
          ...
          if (issue.fields.issuelinks) {
            ...
            issueLinks.forEach(issueLink => {
              ...
              summary =
                status == 'Closed'
                  ? chalk.gray.dim(`jira description (${status})`)
                  : chalk.gray(`jira description (${status})`);
